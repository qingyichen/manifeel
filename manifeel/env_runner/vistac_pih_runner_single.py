...
class ManifeelRunner(BaseImageRunner):
    def __init__(self,
            output_dir,
            n_train=10,
            n_train_vis=3,
        ...
        isaacgym_env = get_isaacgym_env(isaacgym_cfg)
        ...
        def env_fn():
            return MultiStepWrapper(
                VideoRecordingWrapper(
                    SingleIsaacEnvWrapper(isaacgym_env),
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            pbar = tqdm.tqdm(total=self.max_steps, desc=f"Eval IsaacgymRunner {chunk_idx+1}/{n_chunks}", 
...
def test():
    ...
    with hydra.initialize('../config'):
        cfg = hydra.compose('train_diffusion_vistac_pih_workspace')
    ...
    observations = envs.reset(seed=runner.env_seeds)
    print("obs space: ", envs.observation_space)
    print(observations.keys())
    print(observations['left_tactile_camera_taxim'].shape)
    ...
    #     args_list=[(x,) for x in runner.env_init_fn_dills])
    ...
    #     img = np.moveaxis(observations['right_tactile_camera_taxim'][0][0], 0, -1)[:, :, ::-1]
    ...
    #     seed = runner.env_seeds[i]
    #     prefix = runner.env_prefixs[i]
    #     video_path = all_video_paths[i]
    #     print(video_path)
    #     if video_path is not None:
    #         sim_video = wandb.Video(video_path)
    #         # Log a video
    #         video_name = f'{prefix}sim_video_{seed}'
